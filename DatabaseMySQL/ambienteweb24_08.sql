...
CREATE DEFINER=`root`@`localhost` PROCEDURE `cargarEditarProducto` (IN `pIdProducto` INT)   BEGIN
...
WHERE P.id = pIdProducto
;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `cargarEditarUsuario` (IN `pIdUsuario` INT)   BEGIN
SELECT * FROM CLIENTES WHERE id = pIdUsuario;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `editarProducto` (IN `pIdProducto` INT)   BEGIN
SELECT P.url_imagen, P.cantidad_stock, P.precio, P.id as idProducto, C.nombre as nombre_categoria, P.nombre as nombreProducto
FROM productos P
INNER JOIN categorias C ON P.categoria_id = C.id
WHERE P.id = pIdProducto
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `editarProductoMantenimiento` (IN `pNombreProducto` VARCHAR(255), IN `pIdCategoria` INT, IN `pCantidadStock` INT, IN `pPrecioProducto` DECIMAL(10,2), IN `pIdProducto` INT)   BEGIN
UPDATE productos
SET
nombre = pNombreProducto,
precio = pPrecioProducto,
cantidad_stock = pCantidadStock,
categoria_id = pIdCategoria
WHERE id = pIdProducto;

END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `editarUsuarioMantenimiento` (IN `pNombreUsuario` VARCHAR(255), IN `pApellidoUsuario` VARCHAR(255), IN `pCorreoUsuario` VARCHAR(255), IN `pActivoUsuario` BIT, IN `pRolUsuario` INT, IN `pIdUsuario` INT)   BEGIN

UPDATE clientes
SET

nombre = pNombreUsuario,
apellido = pApellidoUsuario,
correo = pCorreoUsuario,
activo = pActivoUsuario,
IdRoles = pRolUsuario
WHERE id = pIdUsuario;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `editarUsuarioMantenimientoM` (IN `pNombreUsuario` VARCHAR(255), IN `pApellidoUsuario` VARCHAR(255), IN `pCorreoUsuario` VARCHAR(255), IN `pActivoUsuario` BIT, IN `pRolUsuario` INT, IN `pIdUsuario` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `eliminarProductoMantenimiento` (IN `pIdProducto` INT)   BEGIN
DELETE FROM productos
WHERE id = pIdProducto;

END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `eliminarUsuarioMantenimiento` (IN `pIdUsuario` INT)   BEGIN
...
(12, 'Fabian', 'Montero', 'fabianja0477@gmail.com', 'Barrio el Carmen', 'Coronado', 'Costa Rica', 'ufide1', b'1', NULL, NULL, 1),
...
--
ALTER TABLE `usuario_carrito`
  MODIFY `usuario_carrito_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=49;
